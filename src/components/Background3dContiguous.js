...
export default class Background3dContiguous extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      cameraPositionIncrementX: false,
      cameraPositionIncrementZ: false,
    };
  }
  ...
  orbit = () => {
    this.camera.position.x += this.state.cameraPositionIncrementX
      ? 0.01
      : -0.01;
    this.camera.position.z += this.state.cameraPositionIncrementZ
      ? 0.01
      : -0.01;

    if (this.state.camera.position.x === -10) {
      this.setState({ cameraPositionIncrementX: true });
    } else if (this.state.camera.position.x === 10) {
      this.setState({ cameraPositionIncrementX: false });
    }

    if (this.state.camera.position.z === -10) {
      this.setState({ cameraPositionIncrementZ: true });
    } else if (this.state.camera.position.z === 10) {
      this.setState({ cameraPositionIncrementZ: false });
    }
  };
